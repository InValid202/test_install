...
class AccountPaymentLine(models.Model):
    ...
    communication_type = fields.Selection(
        selection=[("normal", "Free")], required=False, default="normal"
    )
    ...
    def _compute_wht_move(self):
        """docstring for _compute_wht_move"""
        for rec in self:
            if rec.move_line_id.move_id:
                rec.wht_total = rec.move_line_id.move_id.amount_wht
            else:
                rec.wht_total = 0
